...
public abstract class ServiceMybatis<T extends BaseEntity> implements BaseService<T>{
	...
	public int insert(T record) {
		...
		record.set("delFlag", Constant.DEL_FLAG_NORMAL);
		record.set("createBy", SysUserUtil.getCacheLoginUser().getId()+","+
				SysUserUtil.getCacheLoginUser().getName());
	...
	public int insertSelectiveApp(T record) {
		record.set("createDate",new Date() );
		record.set("updateDate", new Date());
		record.set("delFlag", Constant.DEL_FLAG_NORMAL);
//		record.set("createBy", SysUserUtils.getCacheLoginUser().getId()+","+
//				SysUserUtils.getCacheLoginUser().getName());
	...
	public int updateByPrimaryKey(T record) {
		SysUser sysUser = SysUserUtil.getCacheLoginUser();
		if(sysUser != null){
	...
	* @return 影响行数
	...
	public int saveapp(T record) {
		int count = 0;
		if (record.get("id") == null) {
			count = this.insertSelectiveApp(record);
		} else {
			count = this.updateByPrimaryKeySelective(record);
		}
		return count;
	...
	*/
	 ...
	 * @param params          附加参数
	 * @return  如果唯一返回true,否则 false
	 */
	public boolean checkUnique(Class<?> bean,String checkField,Object value,Object id,boolean isLogicDel,Map<String, Object> params){
		if(params == null){
		...
		if(isLogicDel){
			params.put("del_flag",Constant.DEL_FLAG_NORMAL);
		}
		params.put(checkField,value);
		...
		params.put("t", table.name());
		
		int count = baseMapper.checkUnique(params);
		return  count == 0;
	}
	 ...
	 * @return
	 */
	public boolean checkUnique(Class<?> bean,String checkField,Object value,Object id){
		return this.checkUnique(bean, checkField, value, id,true,null);
	}
	 ...
	 * @return
	 *//*
	public boolean checkUniqueHaveStatus(Class<?> bean,String checkField,Object value,Object id){
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("status", Constant.SYSTEM_COMMON_ENABLE);
		return this.checkUnique(bean, checkField, value, id,true,null);
	}
